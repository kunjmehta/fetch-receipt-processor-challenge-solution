...
This is a Python 3.12 based solution to the Receipt Processor Challenge. It uses FastAPI which is based on the OpenAPI specification for API building, and Pydantic for data validation. The API itself runs on a Uvicorn local server. The code is well documented and comments abound. The solution is also dockerized and the required dependencies are installed from `requirements.txt`

# Run Instructions
Navigate to the main directory of the code containing the Dockerfile. Then, build the docker image

```
docker build -t fetch_image .
```

Run the container

```
docker run -d --name fetch_container -p 8080:8000 fetch_image
```

A new log directory with a timestamped log file will be created at `app/logs/` and the dockerized application will run on the port 8080. To test the API, the default OpenAPI spec page at `localhost:8080/docs` can be accessed and requests can be made as per the defined schema.
